...
if subprocess.run(["/usr/local/bin/python3", "curador_viral.py"]).returncode != 0:
    print("❌ Erro na pesquisa. A parar.")
    sys.exit(1)

# Passo 2 — Corte automático
print("\n✂️  PASSO 2: A criar shorts...")
if subprocess.run(["/usr/local/bin/python3", "curador_auto.py"]).returncode != 0:
    print("❌ Erro ao criar shorts. A parar.")
    sys.exit(1)

# Passo 3 — Publicar no YouTube
print("\n📺 PASSO 3: A publicar no YouTube...")
if subprocess.run(["/usr/local/bin/python3", "publicar_youtube.py"]).returncode != 0:
